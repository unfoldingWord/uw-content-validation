...
describe('checkNotesTSV7DataRow() - ', () => {
  ...
  describe('link tests - ', () => {
    it('should fail invalid doublet link', async () => {
      ...
      const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'RUT', '2', '12', 'from test line', optionalCheckingOptions);
    ...
    it('should fail invalid link path', async () => {
      ...
      const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'GEN', '1', '7', 'from test line', optionalCheckingOptions);
      expect(rawResults.noticeList.length).toBeGreaterThanOrEqual(1);
      expect(rawResults).toMatchSnapshot();
    });
    ...
    it('should succeed with mixed link types', async () => {
      ...
      const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'GEN', '1', '8', 'from test line', optionalCheckingOptions);
    ...
    it('should succeed with dual links', async () => {
      ...
      const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'GEN', '1', '9', 'from test line', optionalCheckingOptions);
      expect(rawResults.noticeList.length).toEqual(1);
    });
    ...
    it('should pass valid verse link', async () => {
      ...
      const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'GEN', '1', '19', 'from test line', optionalCheckingOptions);
  ...
  describe('Original Quote tests - ', () => {
    it('invalid Original Language', async () => {
      ...
      const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'GEN', '1', '1', 'from test line', optionalCheckingOptions);
      expect(rawResults.noticeList.length).toEqual(2);
      expect(rawResults).toMatchSnapshot();
    });
    ...
    it('should find missing Original Quote', async () => {
      ...
      const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'GEN', '1', '3', 'from test line', optionalCheckingOptions);
      expect(rawResults.noticeList.length).toEqual(2);
      expect(rawResults).toMatchSnapshot();
    });
    ...
    it.skip('should fail with valid but high occurrence number', async () => {
      ...
      const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'MAT', '2', '2', 'from test line', optionalCheckingOptions);
      expect(rawResults.noticeList.length).toEqual(1);
      expect(rawResults).toMatchSnapshot();
    });
  ...
  describe('BookID tests - ', () => {
    ...
    it.skip('should find invalid book ID', async () => {
      ...
      let error = false;
      try {
        await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'GIN', '1', '2', 'from test line', optionalCheckingOptions);
        error = false;
      } catch (e) {
        error = true;
      }
  ...
  it('should handle front matter', async () => {
    ...
    const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'GEN', '1', 'intro', 'from test line', optionalCheckingOptions);
  ...
  it('should find invalid Book ID, chapter number, ID, SupportReference, quotes, OccurrenceNote', async () => {
    ...
    const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'GIN', '1', '2', 'from test line', optionalCheckingOptions);
    expect(rawResults.noticeList.length).toEqual(2);
    expect(rawResults).toMatchSnapshot();
  });
  ...
  it('should find invalid newline in single-line field', async () => {
    const chosenLine = "1:2\tv7qw\t\tfigs-imperative\\n\tוְ⁠חֹ֖שֶׁךְ\t1\tThis is a command. By commanding that light should exist, God made it exist. (See: [[rc://*/ta/man/translate/figs-imperative]])";
    const rawResults = await checkNotesTSV7DataRow(username, languageCode, repoCode, chosenLine, 'GEN', '1', '2', 'from test line', optionalCheckingOptions);
    expect(rawResults.noticeList.length).toBeGreaterThanOrEqual(2);
    expect(rawResults).toMatchSnapshot();
  });
