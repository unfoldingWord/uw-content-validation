...
describe('checkTN_TSV9DataRow() - ', () => {
  ...
  describe('link tests - ', () => {
    ...
    it('should fail if SupportReference link differs from link in OccurrenceNote', async () => {
      ...
      const rawResults = await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'GEN', '1', '6', 'that was supplied', optionalCheckingOptions);
      expect(rawResults.noticeList.length).toEqual(2);
      expect(rawResults).toMatchSnapshot();
    });
    ...
    it('should fail invalid link path', async () => {
      ...
      const rawResults = await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'GEN', '1', '7', 'that was supplied', optionalCheckingOptions);
    ...
    it('should succeed with mixed link types', async () => {
      ...
      const rawResults = await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'GEN', '1', '8', 'that was supplied', optionalCheckingOptions);
      expect(rawResults.noticeList.length).toEqual(1);
    });
    ...
    it('should pass valid verse link', async () => {
      ...
      const rawResults = await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'GEN', '1', '19', 'that was supplied', optionalCheckingOptions);
  ...
  describe('Original Quote tests - ', () => {
    it('invalid Original Language', async () => {
      ...
      const rawResults = await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'GEN', '1', '1', 'that was supplied', optionalCheckingOptions);
      expect(rawResults.noticeList.length).toEqual(2);
      expect(rawResults).toMatchSnapshot();
    });
    ...
    it('should find missing Original Quote', async () => {
      ...
      const rawResults = await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'GEN', '1', '3', 'that was supplied', optionalCheckingOptions);
      expect(rawResults.noticeList.length).toEqual(2);
      expect(rawResults).toMatchSnapshot();
    });
  ...
  describe('GL Note tests - ', () => {
    ...
    it('should find bad ellipse', async () => {
      ...
      const rawResults = await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'GEN', '2', '3', 'that was supplied', optionalCheckingOptions);
      expect(rawResults.noticeList.length).toEqual(2);
      expect(rawResults).toMatchSnapshot();
    });
  ...
  describe('BookID tests - ', () => {
    ...
    it.skip('should find invalid book ID', async () => {
      ...
      let error = false;
      try {
        await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'GIN', '1', '2', 'that was supplied', optionalCheckingOptions);
        error = false;
      } catch (e) {
        error = true;
      }
  ...
  it('should skip front matter', async () => {
    ...
    const rawResults = await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'GEN', '1', 'intro', 'that was supplied', optionalCheckingOptions);
    expect(rawResults.noticeList.length).toEqual(0);
  });
  ...
  it('should find mismatched chapter verse', async () => {
    ...
    const rawResults = await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'GEN', '22', '33', 'that was supplied', optionalCheckingOptions);
    expect(rawResults.noticeList.length).toEqual(3);
    expect(rawResults).toMatchSnapshot();
  });
  ...
  it('should find language code instead of asterisk', async () => {
    ...
    const rawResults = await checkTN_TSV9DataRow(languageCode, repoCode, chosenLine, 'EXO', '1', '2', 'that was supplied', optionalCheckingOptions);
    expect(rawResults.noticeList.length).toEqual(1);
    expect(rawResults).toMatchSnapshot();
  });
