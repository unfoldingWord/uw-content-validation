...
const TA_DISPLAY_LINK_REGEX = new RegExp('\\[([^\\]]+?)\\]\\(rc://([^ /]+?)/ta/man/([^ /]+?)/([^ \\]]+?)\\)', 'g'); // [How to Translate Names](rc://en/ta/man/translate/translate-names)
 ...
 * @param {string} bookID
 * @param {string} fieldName, e.g., 'OccurrenceNote' or 'Note' or 'TWLink' or 'MDFile'
...
export async function checkNotesLinksToOutside(languageCode, repoCode, bookID, givenC, givenV, fieldName, fieldText, givenLocation, checkingOptions) {
    ...
    */
    ...
    parameterAssert(fieldName === 'OccurrenceNote' || fieldName === 'Note' || fieldName === 'TWLink' || fieldName === 'MDFile', `checkNotesLinksToOutside: 'fieldName' parameter should be 'OccurrenceNote' or 'Note' or 'TWLink' or 'MDFile' not '${fieldName}'`);
    ...
    // if (linksList1.length) debugLog(`linksList1=${JSON.stringify(linksList1)}`);
    const linksList2 = fieldText.match(GENERAL_LINK2_REGEX) || [];
    // if (linksList2.length) debugLog(`linksList2=${JSON.stringify(linksList2)}`);
    ...
    if (fieldName === 'MDFile') {
        ...
        while ((regexResultArray = TW_INTERNAL_REGEX.exec(fieldText))) {
            ...
            // eslint-disable-next-line no-unused-vars
            let [_totalLink, _displayName, category, article] = regexResultArray;
            ...
            if (!checkingOptions?.disableAllLinkFetchingFlag) {
                ...
                if (!twFileContent)
                    ...
                    else if (0 && checkingOptions?.disableLinkedTWArticlesCheckFlag !== true) {
        ...
        while ((regexResultArray = TA_DISPLAY_LINK_REGEX.exec(fieldText))) {
            debugLog(`  checkNotesLinksToOutside TA_DISPLAY_LINK_REGEX resultArray=${JSON.stringify(regexResultArray)}`);
            taLinkCount += 1;
            processedLinkList.push(regexResultArray[0]); // Save the full link
            parameterAssert(regexResultArray.length === 5, `TA_DISPLAY_LINK_REGEX expected 5 fields (not ${regexResultArray.length})`)
            // eslint-disable-next-line no-unused-vars
            let [_totalLink, _displayName, languageCode, part, article] = regexResultArray;

            if (languageCode !== '*') {
                const characterIndex = TA_DISPLAY_LINK_REGEX.lastIndex - regexResultArray[0].length + 7; // lastIndex points to the end of the field that was found
        ...
        while ((regexResultArray = TA_DOUBLE_BRACKETED_LINK_REGEX.exec(fieldText))) {
            ...
            // eslint-disable-next-line no-unused-vars
            let [_totalLink, languageCode, part, article] = regexResultArray;
        ...
        while ((regexResultArray = ourTWRegex.exec(fieldText))) {
            ...
            const twRepoName = `${languageCode}_tw`;
            // debugLog(`Got twRepoName=${twRepoName}`);
            const filepath = `bible/${regexResultArray[2]}/${regexResultArray[3]}.md`;
            // debugLog(`Got tW filepath=${filepath}`);
