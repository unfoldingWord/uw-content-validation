 ...
 */
export async function internalCheckTN_TSV9Table(username, languageCode, repoCode, bookID, filename, tableText, givenLocation, checkingOptions) {
    /*
    ...
    for (let n = 0; n < lines.length; n++) {
        ...
        else // not the header
        {
            ...
            if (fields.length === NUM_EXPECTED_TN_TSV_FIELDS) {
                ...
                const drResultObject = await checkTN_TSV9DataRow(username, languageCode, repoCode, lines[n], bookID, C, V, ourLocation, checkingOptions);
