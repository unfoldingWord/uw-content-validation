...
function checkUSFMFile(file) {
            ...
            if (rest) {
                if (MARKERS_WITHOUT_CONTENT.indexOf(marker) >= 0)
                    if (isWhitespace(rest))
                        addNotice(541, `Unexpected whitespace '${rest}'`, ` after \\${marker} marker${lineLocation}`);
                    else
                        addNotice(731, `Unexpected content '${rest}'`, ` after \\${marker} marker${lineLocation}`);
                else if (rest[0] == ' ') {
                    let extract = rest.substring(0, 10).replace(/ /g, '␣');
                    if (rest.length > 10) extract += '…';
                    if (isWhitespace(rest))
                        addNotice(542, `Found only whitespace with \\${marker}`, ` being '${extract}'${lineLocation}`);
                    else
                        addNotice(263, `Unexpected leading space(s) for \\${marker}`, ` with '${extract}'${lineLocation}`);
                }

            } else { // nothing following the marker
                if (MARKERS_WITH_COMPULSORY_CONTENT.indexOf(marker) >= 0)
                    addNotice(732, "Expected compulsory content", ` after \\${marker} marker${lineLocation}`);
            }
        } else
            addNotice(730, `Unexpected '${marker}' marker at start of line`, atString);
    }
    ...
    if (lastFiveFilenameChars != '.usfm')
        addNotice(800, `Filename '${file.name}' should end with '.usfm'`, ` (not '${lastFiveFilenameChars}')`);
        ...
        if (pre_BBB_char != '-')
            addNotice(711, "Filename '" + file.name + "' should contain a hyphen before the book code", " (not '" + pre_BBB_char + "')");
        let underlineCount = countOccurrences(file.name, '_');
        if (underlineCount != 0)
            addNotice(712, "Filename '" + file.name + "' should contain no underscore characters", " (found " + underlineCount + ")");
    ...
    try {
        ...
        addNotice(807, "Filename '" + file.name + "' should contain a valid book number", " (not '" + BBB + "')");
    }
    ...
    for (let n = 1; n <= lines.length; n++) {
        ...
        if (!line) {
            // addNotice(555, "Unexpected blank line" + atString);
            continue;
        }
        if (line.indexOf('\r') >= 0)
            addNotice(502, "Unexpected carriageReturn character", atString);
        if (line[0] != '\\')
            addNotice(713, "Expected line to start with backslash not '" + line[0] + "'", atString);
