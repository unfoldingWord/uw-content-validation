    ...
    */
    ...
    if (fields.length === NUM_EXPECTED_QUESTIONS_TSV_FIELDS) {
        ...
        if (V?.length) { // can be undefined if no colon at split above
            if (V.indexOf(',') !== -1) { // verse list, e.g., 9,11 or 17,25-26
                const parts = V.split(',');
                ...
                for (const partRaw of parts) {
                    ...
                    if (/^\d+$/.test(part)) {
                        ...
                        if (a >= b)
                            addNoticePartial({ priority: 808, message: "Bad verse range in list", details: "Second digits should be greater", rowID, fieldName: 'Reference', excerpt: V, location: ourRowLocation });
